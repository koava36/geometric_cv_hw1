...
def pairwise_interpolate_predictions(
    ...
    ckdt_tree = cKDTree(uv_i)
    _, nn_indexes_in_i = ckdt_tree.query(reprojected_j[:, :2], k=nn_set_size)
    ...
    for idx, point_from_j in tqdm(enumerate(reprojected_j)):
        ...
        distances_to_nearest = np.sqrt(np.sum((point_from_j_nns - point_from_j) ** 2, axis=1))
